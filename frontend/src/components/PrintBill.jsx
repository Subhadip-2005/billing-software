  ...
  const handlePrint = () => {
    ...
    style.innerHTML = `
      @media print {
        ...
        #bill-portal { display: block !important; position: fixed; top: 0; left: 0; width: 100%; background: white; }
        @page { margin: 6mm; size: 80mm auto; }
      }
    `;
    document.head.appendChild(style);
    const portal = document.createElement("div");
    portal.id = "bill-portal";
    portal.style.display = "none";
    portal.innerHTML = document.getElementById("bill-print-area").innerHTML;
  ...
  <div style={{ fontFamily: "'Courier New', monospace", fontSize: "11px", color: "#111", width: "100%", maxWidth: "300px", margin: "0 auto" }}>
        ...
        <div key={i} style={{ marginBottom: "5px", borderBottom: "1px dotted #ddd", paddingBottom: "4px" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <span style={{ flex: 3, fontWeight: "bold", fontSize: "10px" }}>
              {item.medicine_name.length > 16 ? item.medicine_name.substring(0, 16) + "..." : item.medicine_name}
            </span>
            <span style={{ flex: 1, textAlign: "center" }}>{item.quantity}</span>
            <span style={{ flex: 1.5, textAlign: "right" }}>Rs.{item.unit_price}</span>
          ...
          <div style={{ color: "#666", fontSize: "9px", paddingLeft: "2px" }}>
            Base: Rs.{itemBase.toFixed(2)} + GST({item.gst_percentage}%): Rs.{itemGst.toFixed(2)}
          </div>
        </div>
      );
    })}
    <div style={{ borderTop: "1px dashed #aaa", marginTop: "4px", paddingTop: "6px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "2px" }}>
        ...
        <span>Rs.{(subtotal || grandTotal + discount).toFixed(2)}</span>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "2px" }}>
        <span style={{ color: "#555" }}>GST (incl.)</span>
        <span>Rs.{(gstTotal || 0).toFixed(2)}</span>
      </div>
      {discount > 0 && (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", borderTop: "2px solid #333", marginTop: "4px", paddingTop: "4px", fontSize: "13px", fontWeight: "bold" }}>
        <span>TOTAL</span>
        <span>Rs.{Number(grandTotal).toFixed(2)}</span>
      </div>
    </div>
    {discount > 0 && (
      <div style={{ margin: "8px 0", padding: "5px 8px", border: "1px dashed #16a34a", borderRadius: "4px", backgroundColor: "#f0fdf4", textAlign: "center" }}>
        <div style={{ color: "#16a34a", fontWeight: "bold", fontSize: "10px" }}>
          You saved Rs.{Number(discount).toFixed(2)} ({discountPercent}% discount)
        </div>
      </div>
    )}
    ...
    <div style={{ borderTop: "2px dashed #333", marginTop: "8px", paddingTop: "8px", textAlign: "center" }}>
      <div style={{ fontSize: "10px", fontWeight: "bold", marginBottom: "3px" }}>Thank you for your purchase!</div>
      <div style={{ fontSize: "9px", color: "#555" }}>Get well soon</div>
      <div style={{ fontSize: "9px", color: "#888", marginTop: "4px" }}>* Medicines once sold will not be taken back *</div>
      <div style={{ fontSize: "9px", color: "#888" }}>* Check expiry before use *</div>
    </div>
  </div>
);
